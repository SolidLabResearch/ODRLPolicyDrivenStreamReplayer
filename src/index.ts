...
async function main() {
    ...
    const frequency = 4;
    if (observation_pointer < sorted_observation_subjects.length) {
        setInterval(() => {
...
async function publish_one_observation(sorted_observation_subjects: string[], store: any, location_to_publish: string, ldp_communication: LDPCommunication) {
    ...
    store.removeQuads(store.getQuads(namedNode(observation_object.subject.value), namedNode('https://saref.etsi.org/core/hasTimestamp'), null, null));
    const time_now = new Date().toISOString();
    store.addQuad(namedNode(observation_object.subject.value), namedNode('https://saref.etsi.org/core/hasTimestamp'), literal(time_now));
    let timestamp = store.getObjects(namedNode(observation_object.subject.value), namedNode('https://saref.etsi.org/core/hasTimestamp', null));
    ...
    observation_pointer++;
    if (observation_pointer === sorted_observation_subjects.length) {
        console.log('All observations replayed');
    }
}
